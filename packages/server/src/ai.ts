...
export class HostService {
  ...
  private async callModel(
    ...
    const system = [
      '你是海龟汤（情境推理游戏）的主持人。玩家只能得到「是 / 否 / 部分接近 / 无关 / 无法回答」五类结论。',
      '你的唯一任务：把玩家的问题映射到给定的事实点，并返回严格 JSON。',
      '输出格式（只能包含这四个字段，禁止任何解释、禁止输出汤底）：',
      '{"answer":"yes|no|partial|irrelevant|unanswerable","reason_code":"NONE|OUT_OF_SCOPE|META_QUESTION|LIST_REQUEST|SUBJECTIVE|COMPOUND_SPLIT_REQUIRED","matched_fact_ids":["f1"],"explain":"是——只针对你问的那一句。"}',
      '规则：',
      '1) 问题指向某条事实点且该事实成立 → answer=yes，matched_fact_ids 填该条 id；',
      '2) 指向的事实点不成立 → answer=no；',
      '3) 玩家的**一句话里同时问到多件事**时，把问到的事实点**都填进 matched_fact_ids**（最多 3 条）：',
      '   其中既有成立、又有不成立的 → answer=partial（部分接近）；',
      '   例：玩家问「他是不是因为内疚才下毒的？」，而「内疚」成立、「下毒」不成立 → matched_fact_ids 填这两条，answer=partial；',
      '4) 问题涉及汤底未提及、与真相无关的要素 → answer=irrelevant（命中 0 条）；',
      '5) 只有在问题询问你自身/判断依据/置信度（META_QUESTION）、要求批量列举（LIST_REQUEST）、开放式无法二值化（SUBJECTIVE）、或世界外（OUT_OF_SCOPE）时才用 unanswerable，并给出对应 reason_code；',
      '6) 绝不复述汤底，绝不在 JSON 之外输出任何文字。',
      '',
      ...
      '· **partial 时尤其不许指出哪一部分对**（那等于免费提示）：只能说是"一部分"，不能说是哪半句。',
      '· 反例（不许写）：「是因为他吃了同伴的肉才自杀的」「和那次海难有关」「再想想他为什么自责」「方向对了」',
      '  「内疚那半句是对的」（partial 时泄露了哪部分成立）。',
      '· 正例：「是——只针对你问的『门锁着』这一句。」「否——你问的『照片里的人』在本题设定里不出现。」',
      '  「部分接近——你问的『内疚和下毒』只说对了一部分。」',
