...
export interface CoreRoom {
  ...
  matchNo: number;
  /**
   * **单人房**：一个人自己推题（主界面的「单人模式」）。
   *
   * 差别只在"多人特有的部分"：
   *  · 没有轮转 —— 每个回合都是你自己，也**没有计时**（倒计时在多人里是为了公平轮换，
   *    一个人时只会平白制造"超时跳过"的噪音，还会把回合数一路烧到上限）；
   *  · 讨论区 / 准备举手 / 投票 / 踢人 / 房主移交 一律不存在（服务端直接拒绝这些动作）；
   *  · 别人无法加入（加入接口对单人房返回 `SOLO_NO_JOIN`）。
   * 其余功能完全一致：判定与说明句、提示、猜汤底、汤底公示、复盘、AI 出题、参数、记录。
   */
  solo?: boolean;
  turn: CoreTurn;
  config: GameConfig;
